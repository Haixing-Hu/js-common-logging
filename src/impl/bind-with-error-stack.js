 ...
 * @private
 */
function bindWithErrorStack(logger, method, level, appender) {
  const prefix = getLoggingPrefix(logger, level);
  logger[method] = (...args) => {
    const stack = new Error().stack;
    const stackLines = stack.split('\n');
    const callerLine = stackLines[1] ? stackLines[1].trim() : 'unknown source';
    appender[method](...fixFirstArgument(prefix, args), `\t(${callerLine})`);
  };
}

export default bindWithErrorStack;
